    ...
    function buildQTree() {
        ...
        if (0.4 > viewZoom) qTree = null;
            ...
            for (i = 0; i < nodelist.length; i++) {
                node = nodelist[i];
                var tst = (20 >= node.size * viewZoom);
                if (node.shouldRender() && !tst) {
...
var node;
        ...
        for (var i = 0;;) {
            ...
            if (flags & 4) {
                for (;;) { // skin name
                    var t = view.getUint8(offset, true) & 0x7F;
                    offset += 1;
                    if (0 === t) break;
    ...
    function sendMouseMove() {
        ...
        if (wsIsOpen()) {
            ...
            if (64 <= msg * msg + b * b && !(0.01 > Math.abs(oldX - X) && 0.01 > Math.abs(oldY - Y))) {
                oldX = X;
                oldY = Y;
                msg = prepareData(21);
                msg.setUint8(0, 16);
    ...
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 !== leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                var scaleFactor = Math.min(0.22 * canvasHeight, Math.min(200, 0.3 * canvasWidth)) / 200;
    ...
    var nCanvas, ctx, mainCanvas, lbCanvas, chatCanvas, canvasWidth, canvasHeight, qTree = null,
        ...
        hideChat = false,
        smoothRender = 0.4,
        nodeX = ~~((leftPos + rightPos) / 2),
        nodeY = ~~((topPos + bottomPos) / 2),
    ...
    Cell.prototype = {
        ...
        movePoints: function() {
            ...
            function qTr(){qTree.retrieve2(n - 5, q - 5, 10, 10, function(a) {
        ...
        drawOneCell: function(ctx) {
            if (this.shouldRender()) {
                ...
                if (this.wasSimpleDrawing && !b)
                    for (c = 0; c < this.points.length; c++) this.points[c].size = this.size;
                ...
                if (0 !== this.id) {
                    ...
                    if ((showName || c) && this.name && this.nameCache && (null === e || -1 === knownNameDict_noDisp.indexOf(skinName))) {
                        ...
                        ctx.drawImage(rnchache, ~~this.x - ~~(m / 2), bb - ~~(h / 2), m, h);
                        bb += rnchache.height / 2 / ratio + 4;
                    }
                    ...
                    if (showMass && (c || 0 === playerCells.length && (!this.isVirus || this.isAgitated) && 20 < this.size)) {
                        ...
                        m = ~~(e.width / ratio);
                        h = ~~(e.height / ratio);
                        ctx.drawImage(e, ~~this.x - ~~(m / 2), bb - ~~(h / 2), m, h);
                    }
                }
                ctx.restore();
            }
